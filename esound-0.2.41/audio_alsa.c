...
int esd_audio_open()
{
    ...
    ret = snd_pcm_plugin_info( alsa_sound_handle, &alsa_pinfo );
    if ( ret ) {
    ...
    snd_pcm_plugin_flush( alsa_sound_handle, alsa_channel );

    /*ret = snd_pcm_channel_params( alsa_sound_handle, &params );*/
    ret = snd_pcm_plugin_params( alsa_sound_handle, &params );
    if ( ret ) {
    ...
    ret = snd_pcm_plugin_prepare( alsa_sound_handle, alsa_channel );
    if ( ret ) {
    ...
    ret = snd_pcm_plugin_setup( alsa_sound_handle, &setup );
    if( ret ) {
...
int esd_audio_write( void *buffer, int buf_size )
{
    ...
    ret = snd_pcm_plugin_status( alsa_sound_handle, &status );
    if( ret ) {
    ...
    if( status.underrun ) {
        ...
        if (snd_pcm_plugin_status( alsa_sound_handle, &status ) < 0 && driver_trace) {
